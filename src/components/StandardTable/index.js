...
class StandardTable extends Component {
    constructor(props) {
        super(props);
        const { columns, code } = props;
        const needTotalList = initTotalList(columns);
        this.tableColumnWidth = 140;
        this.scrollTop = 0;
        this.pageSize = 50;
        this.pageIndex = 1;
        this.distance = 5;
        ...
        this.localColumn =
            user && user.loginName
                ? JSON.parse(localStorage.getItem(`${code}_${user.loginName}_column`))
                : '';
        this.state = {
            selectedRowKeys: [],
            needTotalList,
    ...
    componentDidMount() {
        const { className, code, expandForm, canInput, onExpandRow, data, defaultPageSize } = this.props;
        ...
        if (localdata) {
            ...
            this.columnSort(localdata, fixedIndex);
        }
        if (!className) return;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        // console.log(data,'data',nextProps.data,'this.props.expandForm',this.props.expandForm,nextProps.expandForm)
        return true
    }
    ...
    firstRenderData = props => {
        ...
        const lastPage = data.list.length % this.pageSize; this.pageSize
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (JSON.stringify(columns) !== JSON.stringify(this.props.column)) {
            this.setState({ newColumns: code ? this.resetColumns() : columns }, () => {
                ...
                if (data) {
                    ...
                    this.columnSort(data, fixedIndex);
                }
            });
        }
    ...
    handleTableChange = (pagination, filtersArg, sorter) => {
        // console.log('pagination, filtersArg, sorter,到这路',pagination, filtersArg, sorter)
        const { onPaginationChange, data } = this.props;
        const { formValues } = this.state;
        this.setState({ newData: [], pageHasChange: true });
        const _user=JSON.parse(localStorage.getItem('user'))
        // console.log('user0000000',_user)
        if (pagination.pageSize&&(_user&&_user.loginName)) {
            localStorage.setItem(`${_user.loginName}_pageSize`, pagination.pageSize);
        }
    ...
    render() {
        ...
        // exportBtn ? rest.footer = () => (<Button onClick={this.exportFn} >{transferLanguage('Common.field.export', language)}</Button>) : null
        return (
            <div className={className} style={{ width: id ? '100%' : 'auto' }}>
                <div className={styles.standardTable}>
                    ...
                    // footer={() => {
                    //     let footer = false
                    //     exportBtn ? footer = (<Button onClick={this.exportFn} >{transferLanguage('Common.field.export', language)}</Button>) : null
                    //     return footer
                    // }}
                    />
                    {/* <Setting
                    ...
                    <Button hidden={!exportBtn} style={{ position: 'absolute', bottom: '0px' }} onClick={this.exportFn} >{transferLanguage('Common.field.export', language)}</Button>
                </div>
            </div>
        );
    }
