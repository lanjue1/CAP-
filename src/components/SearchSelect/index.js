...
export default class SearchSelect extends Component {
  ...
  queryData = ({ params = {}, callback } = {}) => {
    const { dispatch, dataUrl, payload, expandRow, filterParent } = this.props;
    const { expandSearchData } = this.state;
    let _params = params
    _params.pageSize = getPageSize() || 10;

    dispatch({
      type: 'common/querySearchData',
      payload: { params: _params.id ? _params : { ..._params, ...payload }, url: dataUrl },
      callback: data => {
        if (!data) return;
        if (expandRow) {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (JSON.stringify(nextProps.selectedData) !== JSON.stringify(this.props.selectedData)) {
      const { url, dispatch } = nextProps;
      if (url) {
        if (
          nextProps.selectedData.length > 0 &&
          Object.keys(nextProps.selectedData[0]).length === 1
        ) {
          this.querySelectData(nextProps.selectedData);
        } else {
