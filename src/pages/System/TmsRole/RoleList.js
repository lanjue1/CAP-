    ...
    const tableButtonsParams = {
      ...
      rightButtons: (
        <Button.Group>
          ...
          > {transferLanguage('Common.field.flushPrivileges', this.props.language)}</Button>
        </Button.Group>
      ),
      buttons: (
        <Button.Group>
    ...
    const rightDrawParams = {
      ...
      buttons: (
        <span>
          <Button.Group>
            {!vehicleAbled_check && (
              <Button onClick={() => this.useVehicleType('abled', 1)}> {transferLanguage('Common.field.enable', this.props.language)}</Button>
            )}
            {vehicleAbled_check && (
              <Button onClick={() => this.useVehicleType('disabled', 1)}>{transferLanguage('Common.field.disable', this.props.language)}</Button>
            )}
            <Button type="primary" onClick={this.handleEdit}>
            {transferLanguage('Common.field.edit', this.props.language)}
            </Button>
          </Button.Group>
        </span>
      ),
