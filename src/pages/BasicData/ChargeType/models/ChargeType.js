...
} from '@/services/billing/chargeType';

export default {
  namespace: 'ChargeType',
  state: {
    
    chargeTypeList: {}, // list列表
    loadDetailList:{},
    loadDetail: {},
   // archivesDetail: {},
    formValues: {},
...
//Effect :Action 处理器，处理异步动作，基于 Redux-saga 实现。
  effects: {
    ...
    *selectChargeType({ payload, callback }, { call, put }) {
      const response = yield call(selectChargeType, payload);
    ...
    *viewLoad({ payload, callback }, { call, put }) {
    ...
    // 3、启用|禁用 货主信息
    *abledSnapshot({ payload, callback }, { call }) {
      const response = yield call(abledSnapshot, payload);
