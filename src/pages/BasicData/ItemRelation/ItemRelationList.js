...
@ManageList
@connect(({ itemRelation, common, component, loading, i18n }) => ({
    itemRelation,
    loading: loading.effects['itemRelation/itemRelationList'],
...
export default class ItemRelationList extends Component {
    ...
    {
        //标题
        title: transferLanguage('partRelation.field.originalItemCode', this.props.language),//'原始料'
        //数据字段
    ...
    {
        title: transferLanguage('partRelation.field.replaceItemCode', this.props.language),//'替代料'
        dataIndex: 'replacePartCode',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    {
        title: transferLanguage('partRelation.field.replaceType', this.props.language),//'替代料类型'
        dataIndex: 'replaceType',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    {
        title: transferLanguage('partRelation.field.beActive', this.props.language),// '状态'
        dataIndex: 'status',
        render: text => <AdSelect data={Status} value={text} onlyRead={true} />,
        width: 100,
    },
    {
        title: transferLanguage('partRelation.field.mtSpe', this.props.language),//'MT Spe'
        dataIndex: 'mtSpe',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    {
        title: transferLanguage('partRelation.field.mt', this.props.language),//'MT'
        dataIndex: 'mt',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    {
        title: transferLanguage('partRelation.field.priority', this.props.language),//'优先级'
        dataIndex: 'priority',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    {
        title: transferLanguage('partRelation.field.topmost', this.props.language),//'最高级别'
        dataIndex: 'topmost',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    ...
    getItemRelationList = (params = {}) => {
        ...
        dispatch({
            type: 'itemRelation/itemRelationList',
            payload: params,
            callback: data => {
                if (!data) return;
                let valueList = [];
    ...
    handleDelete = () => {
        ...
        params.ids = checkIds;
        dispatch({
            type: 'itemRelation/itemRelationDelete',
            payload: params,
            callback: res => {
                this.getItemRelationList(formValues);
            },
    ...
    render() {
        const {
            loading,
            itemRelation: { itemRelationList, itemRelationDetails },
            form,
            isMobile,
            dictObject,
            language,
        ...
        const firstFormItem = (
            <FormItem label={transferLanguage('partRelation.field.originalItemCode', language)}>
                {getFieldDecorator('originalPartCode')(<Input placeholder="" />)}
            </FormItem>
        );
        ...
        const tableButtonsParams = {
            ...
            rightButtons: (<Button.Group>
                <AdButton
                    ...
                    type='primary'
                    text={transferLanguage('partRelation.button.add', language)}
                    code={codes.add}
                />
                <AdButton
                    ...
                    type='danger'
                    text= {transferLanguage('partRelation.button.delete', language)}
                    code={codes.delete}
                />
            </Button.Group>),
            buttons: (
                <Button.Group>
                    <AdButton
                        ...
                        disabled={selectedRows.length > 0 ? false : true}
                        text={transferLanguage('partRelation.button.disable', language)}
                        code={codes.disabled}
                    />
                    <AdButton
                        onClick={() => this.abledStatus('abled')}
                        disabled={selectedRows.length > 0 ? false : true}
                        text={transferLanguage('partRelation.button.enable', language)}
                        code={codes.enable}
                    />
                </Button.Group>
            ),
