import {
    ...
    itemRelationDelete,
    ableOperate,
  } from '@/services/basicData/partException';
  ...
  export default {
    ...
    effects: {
      //替代料关联列表查询：
      *partExceptionList({ payload, callback }, { call, put }) {
        const response = yield call(partExceptionList, payload);
        if (response.code === 0) {
    ...
    *itemRelationDelete({ payload, callback }, { call }){
    ...
    reducers: {
      ...
      show(state, { payload }) {
        return {
          ...state,
          partExceptionList: payload.partExceptionList,
        };
      },
      //详情数据
