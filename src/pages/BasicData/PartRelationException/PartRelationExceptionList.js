...
@ManageList
@connect(({ partRelationException, common, component, loading, i18n }) => ({
    partRelationException,
    loading: loading.effects['partRelationException/partRelationExceptionList'],
...
export default class ItemRelationList extends Component {
    ...
    className = 'partRelationException';
    ...
    {
        title: transferLanguage('WarehouseList.field.country', this.props.language),//'国家'
        dataIndex: 'country',
        render: text => <span title={text}>{text}</span>,
        width: 150,
    },
    ...
    getItemRelationList = (params = {}) => {
        ...
        dispatch({
            type: 'partRelationException/partRelationExceptionList',
            payload: params,
            callback: data => {
                if (!data) return;
                let valueList = [];
    ...
    handleEdit = (e, record) => {
        ...
        dispatch({
            type: 'partRelationException/itemRelationDetails',
            payload: { id },
            callback: res => {
                this.setState({
    ...
    abledStatus = (type, isSingle) => {
        ...
        dispatch({
            type: 'partRelationException/ableOperate',
            payload: params,
            callback: res => {
    ...
    handleDelete = () => {
        ...
        params.ids = checkIds;
        dispatch({
            type: 'partRelationException/itemRelationDelete',
            payload: params,
            callback: res => {
                this.getItemRelationList(formValues);
            },
    ...
    render() {
        const {
            loading,
            partRelationException: { partRelationExceptionList, itemRelationDetails },
            form,
            isMobile,
            dictObject,
            language,
        ...
        const otherFormItem = [
            [
                <FormItem label={transferLanguage('CountryList.title.list', language)}>
                    {getFieldDecorator('country')(<Input placeholder="" />)}
                </FormItem>,
            ],
            [
                <FormItem label={transferLanguage('partRelation.field.mt', language)}>
                    {getFieldDecorator('mt')(<Input placeholder="" />)}
                </FormItem>,
            ],
            ['operatorButtons']
        ];
        ...
        return (
            <Fragment>
                ...
                <StandardTable
                    selectedRows={selectedRows}
                    loading={loading}
                    data={partRelationExceptionList}
