...
@connect(({ LogisticsBilling, loading, component, i18n }) => ({
    LogisticsBilling,
    receiptList: LogisticsBilling.receiptList,
...
@Form.create()
export default class LogisticsBillingList extends Component {
    className = 'LogisticsBillingList';
    constructor(props) {
        super(props);
        this.state = {
            listCol: {
    ...
    abledStatus = (type) => {
        ...
        dispatch({
            type: 'LogisticsBilling/abledOperate',
            payload: params,
            callback: (res) => {
                this.setState({
                    pageLoadding: false
    ...
    render() {
        ...
        const secondFormItem = (
            <AntdFormItem label={transferLanguage('Common.field.status', language)} code="status" {...commonParams}>
                <AdSelect payload={{ code: allDictList.Billing_Status }} />
            </AntdFormItem>
        );
        const otherFormItem = [
            [
        ...
        const tableButtonsParams = {
            ...
            rightButtons: (
                <Button.Group>
                    <AdButton
                        type="primary"
                        onClick={() => this.print('LOGISTIC_BILLING')}
