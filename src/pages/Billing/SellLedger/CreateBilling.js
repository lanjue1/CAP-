...
@connect(({ SellLedger, common, loading, i18n }) => ({

  chargeDetailDetail: SellLedger.chargeDetailDetail,
  buyLedgerDetailList: SellLedger.buyLedgerDetailList,
...
@Form.create()
export default class CreateBillingSell extends Component {
  className = 'CreateBillingSell'
  constructor(props) {
    super(props);
    this.state = {
      checkIds: [],
  ...
  handleSearchFirst = formValues => {
    ...
    const { billDate, warehouseCode, ...value } = formValues
  ...
  handleDetailSearchFirst = formValues => {
    const value = {
      ...
      coNo: formValues.coNoDetail,
      soId: formValues.soIdDetail,
  ...
  getValue = (values, type) => {
    ...
    */
    if(type=='taxAmount'){
      let _value=+values+(+getFieldValue('totalAmount'))-(getFieldValue('adjustAmount')||0)
      setFieldsValue({
    ...
    if(type=='adjustAmount'){
      let taxAmount=+getFieldValue('taxAmount')||0
      let totalAmount=+getFieldValue('totalAmount')
      let _value=taxAmount+totalAmount-values
      setFieldsValue({
  ...
  render() {
    ...
    const otherFormItem = [
      ...
      [
        ...
        <AntdFormItem label={transferLanguage('SellLedger.field.closeType', language)} code="closeType" {...commonParams}>
          <AdSelect data={SelectCloseType} isExist />
        </AntdFormItem>,
      ],
      [
    ...
    const secondDetailFormItem = (
      <AntdFormItem label={transferLanguage('ChargeDetail.field.coNo', language)} code="coNoDetail" {...commonParams}>
        <TextArea rows={1} />
      </AntdFormItem>
    ...
    const otherDetailFormItem = [
      [
        <AntdFormItem label={transferLanguage('ChargeDetail.field.soId', language)} code="soIdDetail" {...commonParams}>
          <TextArea rows={1} />
        </AntdFormItem>
