...
import { columnsBilling, BillingStatus } from '../LogisticsBilling/utils'
...
@connect(({ LogisticsLedger, loading, component, i18n }) => ({
    LogisticsLedger,
    chargeDetailList: LogisticsLedger.chargeDetailList,
    billinglList: LogisticsLedger.billinglList,
...
@Form.create()
export default class LogisticsLedgerList extends Component {
    className = 'LogisticsLedgerList';
    ...
    addBilling = () => {
        ...
        const { dispatch } = this.props
        dispatch({
            type: 'LogisticsLedger/selectBillinglList',
            payload: { status: 'OPEN' },
            callback: () => {
    ...
    handleSearch = e => {
        ...
        form.validateFields((err, fieldsValue) => {
            ...
            if (_createDate && _createDate.length > 0) {
                value.createDateStart = moment(_createDate[0]).format(dateFormat)
                value.createDateEnd = moment(_createDate[1]).format(dateFormat)
                value.createDateStart +=' 00:00:00'
                value.createDateEnd +=' 23:59:59'
            } else {
    ...
    render() {
        ...
        const secondFormItem = (
            <AntdFormItem label={transferLanguage('Common.field.status', language)} code="status" {...commonParams}>
                <AdSelect data={LedgerStatus} isExist />
            </AntdFormItem>
        );
        const otherFormItem = [
            [
        ...
        const tableButtonsParams = {
            ...
            rightButtons:(
                <Button.Group>
                    <AdButton 
                    onClick={() => router.push('/billing/chargeDetail/addChargeDetail')}
