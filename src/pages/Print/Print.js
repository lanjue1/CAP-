...
export default class Print extends Component {
    ...
    componentDidMount() {
        const { dispatch, match: { params },printArr,location:{query}} = this.props;
        console.log('?queryquery,',query)
        const _params={ ids:printArr, type: params.type,}
        if (query) {_params.status=query.status}
        dispatch({
            type: 'Picking/getBiReport',
            payload: _params,
            callback: url => {
                this.setState({ url });
            },
